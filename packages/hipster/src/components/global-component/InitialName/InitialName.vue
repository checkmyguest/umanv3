<template>
    <div class="container" v-if="hasFullname" :alt="fullname" :title="fullname">
      ...
      <div v-if="showText">
        <p class="whitespace-nowrap capitalize" :class="{ 'text-blue-600': email }">
          {{ fullname }}
        </p>
  ...
  export default {
    ...
    computed: {
      hasFullname() {
        return this.fullname && this.fullname.length && this.fullname.trim().length
      },
      initials() {
        let initials = this.fullname!
          .trim()
          .split(" ")
