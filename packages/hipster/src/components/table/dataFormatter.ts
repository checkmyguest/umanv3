...
export function dataFormatterEvent(data: IData): IDataEvent[]{
    ...
    let state: string = "";
    data.forEach((item) =>{
        item.origin = originFormat.find((element) => element.value === item.origin)!.label;
        item.status = statusFormat.find((element) => element.value === item.status)!.label;
        if(dayjs().diff(item.departure_date) > 0 && dayjs(item.arrival_date).diff(dayjs()) > 0)
            state = "En cours";
        else if(dayjs().diff(item.departure_date) < 0)
            state = "À venir";
        else
            state = "Terminée";
        ...
        item.guestProfileStatus = guestProfileFormat.find((element) => element.value === item.guestProfileStatus)!.label;
        item.caution = cautionFormat.find((element) => element.value === item.caution)!.label;
        dataEvent.push({
            event: {
                id: item.id,
                notes: item.notes
